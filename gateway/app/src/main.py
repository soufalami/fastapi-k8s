...
def get_msg(book:Book, client:Client) -> str: 
    if not client: 
        msg="Hazte cliente para conseguir maravillosos descuentos."
        if book.basic_discount > 0:
            msg+=f' En este libro podrías haber conseguido un {book.basic_discount:.0%} solo por ser cliente.'
    else: 
        msg=f'{client.name} gracias por seguir comprando con nosotros.'
        if book.basic_discount > 0 and client.type == "Basic":
            msg+=f' En este libro has conseguido un {book.basic_discount:.0%} solo por ser cliente.'
        elif book.premium_discount > 0 and client.type == "Premium":
            msg+=f' En este libro has conseguido un {book.premium_discount:.0%} por ser cliente Premium.'
    return msg
